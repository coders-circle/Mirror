...
{
    memcpy(m_rawBGRAData.data(), bgraData, (m_w*m_h) << 2);
...
void FrameRenderer::Resize(int w, int h)
{
    m_w = w;
    m_h = h;
    Control::Resize(m_w, m_h);
    if (m_drawingSurface)
    {
        cairo_surface_destroy(m_drawingSurface);
    }
    m_rawBGRAData.resize(4 * m_w* m_h);
    m_drawingSurface = cairo_image_surface_create_for_data(m_rawBGRAData.data(), CAIRO_FORMAT_ARGB32, m_w, m_h, m_w * 4);
}
