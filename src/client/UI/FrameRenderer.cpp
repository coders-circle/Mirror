...
void FrameRenderer::SetRGBData(unsigned char* rgbData)
{
    ...
    {
        m_bgraData[i1 + 0] = rgbData[i2 + 2];
        m_bgraData[i1 + 1] = rgbData[i2 + 1];
        m_bgraData[i1 + 2] = rgbData[i2 + 0];
        m_bgraData[i1 + 3] = 255;
    }
...
{
    memcpy(m_bgraData, bgraData, m_w*m_h * 4);
    gtk_widget_queue_draw_area(m_drawingArea, 0, 0, m_w, m_h);
}
...
gboolean FrameRenderer::OnDraw(GtkWidget* widget, cairo_t* cr, gpointer frPointer)
{
    ...
    cairo_set_source_surface(cr, fr->m_surface, 0, 0);
    cairo_paint(cr);
    return FALSE;
}
...
void FrameRenderer::Set(GtkWidget* fixed, int x, int y, int fw, int fh)
{
    ...
    if (m_bgraData)      delete m_bgraData;
    if (m_surface)     cairo_surface_destroy(m_surface);

    
    m_bgraData       = new unsigned char[4*m_w*m_h];
    m_drawingArea   = gtk_drawing_area_new();
    m_surface = cairo_image_surface_create_for_data(m_bgraData, CAIRO_FORMAT_ARGB32, m_w, m_h, m_w*4);
